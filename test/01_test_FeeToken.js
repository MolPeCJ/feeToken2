...
describe('FeeToken Test', () => {
  ...
  it('should emit wallet address', async () => {
    ...
    const feeToken = await feeTokenInstance.deploy(ownerTokens.address, wallet.address, 
    process.env.TOKEN_NAME, process.env.TOKEN_SYMBOL)

    expect(wallet.address).to.emit("NewFee").withArgs(constants.ZERO_ADDRESS, wallet.address);
  });
});*/
...
describe('Other tests', () => {
  ...
  it('Test: should transfer', async () => {
    tx = await feeToken.connect(ownerTokens).transfer(addr1.address, 100000);
    ...
    expect(tx).to.emit(feeToken, "Transfer").withArgs(ownerTokens.address, addr1.address, 100000);
